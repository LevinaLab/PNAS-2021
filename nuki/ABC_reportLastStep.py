...
# Load Posterioes
Post = []
Eps = [0.05,0.1,0.2,0.3,0.5,0.7,0.8,0.92]#[0.05,0.2,0.5,0.7,0.8,0.92]#,0.5,0.8]#[0.05,0.1,0.2,0.3,0.5,0.65,0.8,0.95]
for epsilon in Eps:
...
try:
    ...
    for i,eps in enumerate(Eps):
        AllData[:,:,i] = get_AllFeatures(eps,data, network_type =
                                         'nuki_LastStep')
    np.save('ABC_results/AllDataReport.npy',AllData)


#Load the static model
StaticResults = np.load('static/Features.npy', allow_pickle = True)
...
with PdfPages('/home/ovinogradov/Projects/EI_project/ABC_bursting/figs/report3.pdf') as pdf:
    ...
    if report:
        pdf.savefig()
    if paper_figures:
        plt.savefig('figs/Figure5E.pdf',fmt='pdf')
        plt.close()
    if show:
        plt.show(block= False)
    ...
    for i,eps in enumerate(Eps):
        if sim_type=='balanced':
            ...
            in_deg = None
        res= get_activity(eps,data,sim_type=sim_type,network_type ='nuki_LastStep',plot=False)
    ...
    for i,eps in enumerate(Eps):
        for index in range(50):
            res= get_activity(eps,data,sim_type='ML',network_type='nuki_LastStep', index=index,plot=False, verbosity =False)
    ...
    if report:
        pdf.savefig()
    if paper_figures:
        plt.savefig('figs/Figure2.pdf',fmt= 'pdf')
#        plt.close()


    """ Figure 5BC"""
    Ns = na([np.sum(np.isnan(IBIs[eps])==False) for eps in IBIs.keys()])
    sns.set_context('talk')
    plt.figure(figsize=(10, 5))
    plt.subplot(121)
    plt.errorbar(na(origEps2)*100,np.nanmean(IBIs,0),np.nanstd(IBIs,0)/np.sqrt(Ns),fmt = 'bo-',label='Data',zorder=0)
    for i,eps in enumerate(Eps):
        plt.plot([eps*100]*50, AllData[0,:,i]/1000,'.',color = 'salmon',alpha =0.2)
        plt.errorbar([eps*100],
                     np.mean(AllData[0,:,i]/1000),np.std(AllData[0,:,i]/1000)/np.sqrt(50),fmt='-or',alpha =0.9,zorder=10)
    plt.errorbar([eps*100],
                 np.mean(AllData[0,:,i]/1000),np.std(AllData[0,:,i]/1000)/np.sqrt(50),fmt='-or',alpha =0.9,zorder=10,label ='Model with adaptation')
    plt.plot(na(Eps)*100,mapData[0,:]/1000,'*',label = 'MAP')
    staticIBI = [StaticResults.item().get(eps)[0] for eps in Eps]
#    plt.plot(na(Eps)*100 ,na(staticIBI)/1000,'o-',color = 'dimgray', label ='Model without adaptation')
    plt.ylabel('IBI(s)')
    plt.xlabel('Inhibitory percentage (%%)')
    plt.legend(fontsize =14)
    sns.despine()
    plt.subplot(122)
    plt.errorbar(na(origEps2)*100,np.nanmean(CVs,0),np.nanstd(CVs,0)/np.sqrt(Ns),fmt = 'bo-',label='Data',zorder=0)
    plt.plot()
    for i,eps in enumerate(Eps):
        plt.plot([eps*100]*50, AllData[1,:,i],'.',color = 'salmon',alpha =0.2)
        plt.errorbar([eps*100],
                     np.mean(AllData[1,:,i]),np.std(AllData[1,:,i])/np.sqrt(50),fmt='-or',alpha =0.9,zorder=10)
    plt.errorbar([eps*100],
                 np.mean(AllData[1,:,i]),np.std(AllData[1,:,i])/np.sqrt(50),fmt='-or',alpha =0.9,zorder=10,label ='Model with adaptation')
    plt.plot(na(Eps)*100,mapData[1,:],'*', label ='MAP')

#    staticCV = [StaticResults.item().get(eps)[1] for eps in Eps]
#    plt.plot(na(Eps)*100 ,staticCV,'o-',color = 'dimgray', label ='Model without adaptation')
    ...
    if show:
        plt.show()
    if paper_figures:
        plt.savefig('figs/Figure5BC.pdf',fmt='pdf')
    if report:
        pdf.savefig()
    ...
    plt.errorbar(na(origEps2)*100,np.nanmean(Amps/Amps[0.1].mean(),0),np.nanstd(Amps/Amps[0.1].mean(),0)/np.sqrt(Ns),fmt='-o')
    staticAmps = [StaticResults.item().get(eps)[2] for eps in Eps]
    plt.plot(na(Eps)*100 ,na(staticAmps)/staticAmps[1],'o-',color = 'dimgray', label ='Model without adaptation')
    plt.xlabel('Inhibitory percentage (%)')
    ...
    if show:
        plt.show(block = False)
    if paper_figures:
        plt.savefig('figs/Figure5D.pdf',fmt='pdf')
    if report:
        pdf.savefig()
